...
export const DEFAULT_VALIDATION_MESSAGES: MessageCollection = {
	...
	messages: {
		...
		minlength:
			'The length of {key} must be greater than or equal to {requiredLength}.',
		maxlength:
			'The length of {key} must be less than or equal to {requiredLength}.',
		...
		datetime: '{key} must be a valid date/time value.',
		dateOnly: '{key} must be a valid date value.',
		...
		noQuotesOrSlashes:
			'{key} must not contain the characters \', ", \\ or /.',
	},
